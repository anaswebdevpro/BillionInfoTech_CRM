...
import { IB_TRANSACTION } from "../../../../../api/api-variable";
...
const PartnerCommissionsTable: React.FC = () => {
  // State for Partner Commissions table
    ...
    async (
      ...
      try {
        ...
        if (response) {
          ...
          // console.log('Partner Commissions Data Set Successfully');
        } else {
          console.warn("Partner Commissions: No response received");
          setError("No data received from Partner Commissions API");
        }
      } catch (error) {
  ...
  // Search handler - now just sets the search value, debounce will handle the API call
  ...
  const formatCellValue = (
    ...
    if (value === null || value === undefined) {
      return columnIndex === 2 ? "N/A" : "-"; // Order Ref ID column shows 'N/A'
    }
  ...
  const headers = ["ID", "Ticket ID", "Order Ref ID", "Amount", "Created On"];

  return (
    <Card className="shadow-lg hover:shadow-xl transition-shadow duration-300">
      <div className={`px-6 py-4 border-b border-${COLORS.BORDER}`}>
        <h3
          className={`text-xl font-bold text-${COLORS.SECONDARY} flex items-center gap-2`}
        >
        ...
        <p className={`text-sm text-${COLORS.SECONDARY_TEXT} mt-1`}>
          {isLoading
            ? "Loading..."
            : `${data?.recordsTotal || 0} total transactions`}
        </p>
      </div>
      ...
      <div className="p-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div className="flex items-center gap-3">
            <label
              className={`text-sm font-semibold text-${COLORS.SECONDARY_TEXT}`}
            >
              Show
            </label>
            <select
            ...
            <thead className="bg-gray-50">
              <tr>
                {headers.map((header, index) => (
                  <th
                    key={index}
                    className={`text-left py-4 px-6 font-bold text-sm text-${COLORS.SECONDARY} uppercase tracking-wider`}
                  >
                    {header}
                  </th>
                ))}
              ...
              {isLoading ? (
                <tr>
                  <td
                    ...
                    className={`text-center py-12 text-${COLORS.SECONDARY_TEXT}`}
                  >
                    <div className="flex flex-col items-center gap-3">
                      <div className="w-8 h-8 border-4 border-blue-200 border-t-blue-600 rounded-full animate-spin"></div>
                      <div
                        className={`text-lg font-semibold text-${COLORS.SECONDARY_TEXT}`}
                      >
