...
const AdminClientsLeads: React.FC = () => {
  ...
  const clientsData = [
    ...
    { id: 3, name: 'Bob Johnson', email: 'bob@example.com', phone: '+1234567892', status: 'Active', type: 'Client', joined: '2024-01-20' },
    { id: 4, name: 'Alice Brown', email: 'alice@example.com', phone: '+1234567893', status: 'Inactive', type: 'Lead', joined: '2024-02-05' },
  ];
    ...
    <div className="p-6">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className={`text-2xl font-bold text-${COLORS.SECONDARY}`}>Clients & Leads</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        <div className={`bg-${COLORS.WHITE} p-6 rounded-lg border border-${COLORS.BORDER}`}>
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className={`text-2xl font-semibold text-${COLORS.SECONDARY}`}>2,847</p>
            </div>
          </div>
        </div>
        ...
        <div className={`bg-${COLORS.WHITE} p-6 rounded-lg border border-${COLORS.BORDER}`}>
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className={`text-${COLORS.SECONDARY_TEXT} text-sm`}>New Leads</p>
              <p className={`text-2xl font-semibold text-${COLORS.SECONDARY}`}>1,234</p>
            </div>
          </div>
        </div>
        ...
        <div className={`bg-${COLORS.WHITE} p-6 rounded-lg border border-${COLORS.BORDER}`}>
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className={`text-${COLORS.SECONDARY_TEXT} text-sm`}>Active Clients</p>
              <p className={`text-2xl font-semibold text-${COLORS.SECONDARY}`}>2,156</p>
            </div>
          </div>
        </div>
        ...
        <div className={`bg-${COLORS.WHITE} p-6 rounded-lg border border-${COLORS.BORDER}`}>
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className={`text-2xl font-semibold text-${COLORS.SECONDARY}`}>68%</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={`bg-${COLORS.WHITE} p-6 rounded-lg border border-${COLORS.BORDER} mb-6`}>
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex gap-3">
            ...
            <button className={`px-4 py-2 bg-${COLORS.PRIMARY} text-${COLORS.WHITE} rounded-lg hover:bg-green-700 transition-colors flex items-center gap-2`}>
          ...
          <table className="w-full">
            <thead>
              <tr className={`border-b border-${COLORS.BORDER}`}>
                ...
                <th className={`text-left p-4 font-semibold text-${COLORS.SECONDARY_TEXT}`}>Phone</th>
                ...
                <th className={`text-left p-4 font-semibold text-${COLORS.SECONDARY_TEXT}`}>Joined</th>
                <th className={`text-left p-4 font-semibold text-${COLORS.SECONDARY_TEXT}`}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {clientsData.map((client) => (
                <tr key={client.id} className={`border-b border-${COLORS.BORDER} hover:bg-${COLORS.SECONDARY_BG} transition-colors`}>
                  <td className="p-4">
                    <div className="font-medium text-gray-900">{client.name}</div>
                  </td>
                  <td className={`p-4 text-${COLORS.SECONDARY_TEXT}`}>{client.email}</td>
                  <td className={`p-4 text-${COLORS.SECONDARY_TEXT}`}>{client.phone}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${
                      client.status === 'Active' ? `bg-${COLORS.PRIMARY_BG_LIGHT} text-${COLORS.PRIMARY_TEXT}` :
                      client.status === 'Pending' ? `bg-${COLORS.YELLOW_BG} text-${COLORS.YELLOW_TEXT}` :
                      'bg-red-50 text-red-700'
                    }`}>
                      {client.status}
                    </span>
                  </td>
                  <td className={`p-4 text-${COLORS.SECONDARY_TEXT}`}>{client.joined}</td>
                  <td className="p-4">
