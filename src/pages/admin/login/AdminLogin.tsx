...
import { ADMIN_LOGIN } from "../../../../api/api-variable";
import { COLORS, GRADIENTS } from "@/constants/colors";
import { Button, Input, ShimmerLoader } from "@/components";
 ...
 */
const AdminLoginPage: React.FC<LoginPageProps> = ({ onSwitchToSignup }) => {
      ...
      try {
        ...
        apiRequest({
          ...
          .then((response: any) => {
            ...
            // If API does not return user_type, show an error and do not proceed
            if (!user_type) {
              setIsLoading(false);
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex">
        ...
        >
          <ShimmerLoader variant="form" width={400} height={500} />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex">
      ...
      >
        <div
          className={`w-full max-w-md border border-${COLORS.BORDER} bg-${COLORS.WHITE} rounded-lg ${COLORS.SHADOW} p-8`}
        >
          ...
          <form onSubmit={formik.handleSubmit} className="space-y-6">
            ...
            >
              {isLoading ? (
                <>
                  <svg
                    className={`animate-spin h-5 w-5 mr-2 text-${COLORS.PRIMARY}`}
                    viewBox="0 0 24 24"
                  >
                    <circle
