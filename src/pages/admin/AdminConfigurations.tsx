...
import { Settings, Globe, Users, Shield, Key, Database, Server, Cog } from 'lucide-react';
    ...
    <div className="p-6">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className={`text-2xl font-bold text-${COLORS.SECONDARY}`}>System Configurations</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        <div className={`bg-${COLORS.WHITE} p-6 rounded-lg border border-${COLORS.BORDER}`}>
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className={`text-${COLORS.SECONDARY_TEXT} text-sm`}>Total Configs</p>
              <p className={`text-2xl font-semibold text-${COLORS.SECONDARY}`}>47</p>
            </div>
          </div>
        </div>
        ...
        <div className={`bg-${COLORS.WHITE} p-6 rounded-lg border border-${COLORS.BORDER}`}>
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className={`text-${COLORS.SECONDARY_TEXT} text-sm`}>Security Level</p>
              <p className={`text-2xl font-semibold text-${COLORS.SECONDARY}`}>High</p>
            </div>
          </div>
        </div>
        ...
        <div className={`bg-${COLORS.WHITE} p-6 rounded-lg border border-${COLORS.BORDER}`}>
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className={`text-${COLORS.SECONDARY_TEXT} text-sm`}>Server Status</p>
        ...
        <div className={`bg-${COLORS.WHITE} p-6 rounded-lg border border-${COLORS.BORDER}`}>
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className={`text-${COLORS.SECONDARY_TEXT} text-sm`}>Last Backup</p>
              <p className={`text-2xl font-semibold text-${COLORS.SECONDARY}`}>2h ago</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {configSections.map((section, index) => {
          ...
          return (
            <div key={index} className={`bg-${COLORS.WHITE} rounded-lg border border-${COLORS.BORDER}`}>
              <div className="p-6 border-b border-gray-200">
                <div className="flex items-center">
                  <div className={`bg-${COLORS.PRIMARY_BG_LIGHT} p-2 rounded-lg mr-3`}>
                    <IconComponent className={`h-5 w-5 text-${COLORS.PRIMARY}`} />
                  </div>
                  <h2 className={`text-lg font-semibold text-${COLORS.SECONDARY}`}>{section.title}</h2>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  {section.configs.map((config, configIndex) => (
                    <div key={configIndex} className="flex items-center justify-between">
                      <div>
                        <label className={`font-medium text-${COLORS.SECONDARY}`}>{config.name}</label>
                        <p className={`text-sm text-${COLORS.SECONDARY_TEXT}`}>Current: {config.value}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        {config.type === 'toggle' ? (
                          <button className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                            config.value === 'Enabled' ? `bg-${COLORS.PRIMARY}` : `bg-${COLORS.GRAY}`
                          }`}>
      ...
      {/* API Settings */}
      <div className={`bg-${COLORS.WHITE} rounded-lg border border-${COLORS.BORDER} mt-6`}>
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <div className={`bg-${COLORS.PRIMARY_BG_LIGHT} p-2 rounded-lg mr-3`}>
              ...
              <h2 className={`text-lg font-semibold text-${COLORS.SECONDARY}`}>API Configuration</h2>
            </div>
            <button className={`bg-${COLORS.PRIMARY} text-${COLORS.WHITE} px-4 py-2 rounded-lg hover:bg-green-700 transition-colors text-sm`}>
              Generate New Key
            </button>
          </div>
        </div>
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className={`block font-medium text-${COLORS.SECONDARY} mb-2`}>API Key</label>
              <div className="flex items-center gap-2">
                <input
                  type="text"
                  value="sk_live_***************************"
            ...
            <div>
              <label className={`block font-medium text-${COLORS.SECONDARY} mb-2`}>Webhook URL</label>
              <div className="flex items-center gap-2">
                <input
                  type="text"
