    ...
    onSubmit: async (values) => {
      try {
        ...
        if (responseData.response === false) {
          enqueueSnackbar(responseData.message || "Transfer failed!", {
            variant: "error",
          });
        } else if (response) {
          ...
          await fetchData();
          enqueueSnackbar(
            responseData.message || "Transfer completed successfully!",
            { variant: "success" }
          );
        }
      } catch (error) {
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">
            {apiData?.title || "Internal Transfer"}
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          >
            <form onSubmit={formik.handleSubmit} className="space-y-6">
              ...
              <div>
                <label
                  className={`block text-sm font-medium text-${COLORS.SECONDARY_TEXT} mb-1`}
                >
                ...
                <select
                  ...
                  className={`block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-${
                    COLORS.PRIMARY_BG
                  } focus:border-${COLORS.PRIMARY_BG} ${
                ...
                >
                  ...
                  {apiData?.from_account.map((account, index) => (
                    <option
                      ...
                      value={
                        account.wallet ||
                        `${account.account_number}-${account.slug}`
                      }
                    >
                      {account.wallet
              ...
              <div>
                ...
                >
                  ...
                  {apiData?.to_account.map((account, index) => (
                    <option
                      key={index}
                      value={`${account.account_number}-${account.slug}`}
                    >
                      Account {account.account_number} ({account.slug})
                    </option>
                  ))}
          ...
          <Card
            ...
            subtitle="Your available trading accounts."
          >
            {isLoading ? (
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-right">
                    <p className="font-semibold text-green-600">
                      ${parseFloat(apiData.main_wallet).toLocaleString()}
                    </p>
                ...
                <div className="flex justify-between items-center p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-right">
                    <p className="font-semibold text-green-600">
                      ${parseFloat(apiData.profit_wallet).toLocaleString()}
                    </p>
                    <p className="text-sm text-gray-500">USD</p>
                  </div>
                </div>
